...
import cn.star.client.mod.mods.render.SelectBlockMod;
    ...
    public void drawSelectionBox(EntityPlayer player, MovingObjectPosition movingObjectPositionIn, int p_72731_3_, float partialTicks)
    {
        if (p_72731_3_ == 0 && movingObjectPositionIn.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
        {
            ...
            if (block.getMaterial() != Material.air && this.theWorld.getWorldBorder().contains(blockpos))
            {
                ...
                AxisAlignedBB axisAlignedBB = block.getSelectedBoundingBox(this.theWorld, blockpos).expand(0.0020000000949949026D
                        , 0.0020000000949949026D
                        , 0.0020000000949949026D).offset(-d0, -d1, -d2);
                Color color = Client.modManager.getByClass(SelectBlockMod.class).isEnable()
                        ? new Color(20, 255, 255, 89)
                        : new Color(0, 0, 0, 102);
                GlStateManager.color(color.getRed() / 255f
                        , color.getGreen() / 255f
                        , color.getBlue() / 255f
                        , color.getAlpha() / 255f);
                if (Client.modManager.getByClass(SelectBlockMod.class).isEnable()) {
                    UtilTools.drawCube(axisAlignedBB);
                } else {
